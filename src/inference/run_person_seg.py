...
def process_frame(frame_path: Path, video_id: int, frame_number: int, 
    ...
    """
    Process a single frame for person detection, logging detections 
    to the PersonClassifications table.
    
    Returns:
    -------
    int: Number of persons detected (used by the calling process_video function).
    """
    # Read frame
    ...
    # Run YOLO detection (using segmentation model for general detection)
    # verbose=False suppresses standard output log lines during loop execution
    results = model.predict(frame, verbose=False)
    
    person_count = 0
    
    # Iterate through results (usually one result per image)
    for result in results:
        # Check for bounding box detections
        if result.boxes is not None:
            # We assume YOLO is configured to detect 'person' as class 0,
            # or the trained model only detects people.
            
            for box in result.boxes:
                confidence = box.conf[0].cpu().numpy()
                # class_id = int(box.cls[0].cpu().numpy()) # Not needed for simple presence check
                
                # Check confidence threshold if needed, but for simplicity, we log all boxes found.
                
                # Insert detection into PersonClassifications table
                # We log the detection details required by your database schema.
                cursor.execute('''
