...
LOG_FILE_PATH = Inference.BOOK_LOG_FILE_PATH
...
def process_video(
    ...
    """
    Process a video for book detection
...
def main(video_list: List[str], frame_step: int = 10):
    """
    Main function to process videos for book detection
    ...
    # Load pretrained yolo model for object (book) detection
    model = YOLO(BookConfig.MODEL_NAME)
    ...
    conn.close()
    logging.info("Book detection processing completed.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run YOLO book detection on extracted frames")
