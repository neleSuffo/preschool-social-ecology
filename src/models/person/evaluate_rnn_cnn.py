...
Usage:
  python evaluate_rnn_cnn_v2.py --model_path /path/to/best.pth
  python evaluate_rnn_cnn_v2.py --diagnose  # Run diagnostics on test set
"""
...
def main():
    ...
    parser.add_argument('--diagnose', action='store_true', help='Run diagnostics on test set')
    ...
    if args.diagnose:
        print("\nRunning diagnostics on test data...")
        diagnostics = diagnose_learning_issues((cnn, rnn_model), test_loader, device)
        ...
        # Save diagnostics
        with open(os.path.join(eval_dir, 'test_diagnostics.json'), 'w') as f:
